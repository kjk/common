...
func appendToFile(file *os.File, data []byte, sync bool) (int64, error) {
...
func serializeRecord(rec *Record) string {
	...
	if rec.Meta == "" {
		return fmt.Sprintf("%s %d %d %s\n", offsetStr, rec.Size, t, rec.Kind)
	}
	return fmt.Sprintf("%s %d %d %s %s\n", offsetStr, rec.Size, t, rec.Kind, rec.Meta)
}
...
func (s *Store) appendRecord(kind string, meta string, data []byte, timestampMs int64) error {
	...
	_, err = appendToFile(s.indexFile, []byte(indexLine), s.SyncWrite)
	if err != nil {
		return err
	}
...
func (s *Store) appendToDataFile(data []byte) error {
	...
	if nWritten, err := appendToFile(s.dataFile, data, s.SyncWrite); err != nil {
		return err
	} else {
...
func ParseIndexLine(line string, rec *Record) error {
	...
	rec.Size, err = strconv.ParseInt(parts[1], 10, 64)
	if err != nil {
